...
def random_split_list(
    input_list: list, perc_part2: float, perc_part3: float = 0
) -> tuple:
    """
    Splits the input_list in 2 or 3 parts randomly according to the passed percentages
    """
    assert type(input_list) is list and len(input_list) > 1, "Unvalid input list"
    ...
    assert (
        set(part1).isdisjoint(set(part3))
...
def save_metrics_to_csv(
    filename: str, metrics: dict[str, float], metrics_order
) -> None:
    try:
    ...
    new_df = pd.DataFrame(metrics, index=[0]).reindex(columns=metrics_order)
